...
<h1 class="block text-lg text-gray-500 font-bold mb-1 pr-4">Registrate</h1>
<form action="{{ url('register') }}" method="POST" class="p-2 ">
    @csrf
    <div class="p-2 flex-col">

        <!-- Nombre -->
        <div >
            <label for="name" class="block text-gray-500 font-bold mb-1 pr-4">
                Nombre
            </label>
            <input required type="text" name="name" class="bg-gray-200 border-2 border-gray-200 rounded w-500 py-2 px-4 text-gray-700 focus:outline-none focus:bg-white focus:border-purple-500">
        </div>
        @error('name')
        ...
        <div >
            ...
            <input required type="email" name="email" class="bg-gray-200 border-2 border-gray-200 rounded w-500 py-2 px-4 text-gray-700 focus:outline-none focus:bg-white focus:border-purple-500">
        </div>
        @error('email')
            <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
        @enderror
